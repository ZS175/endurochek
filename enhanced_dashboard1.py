...
categories = {
    'Двигатель': {
        ...
        'image': "https://sc04.alicdn.com/kf/H51e8ce7322244ee6816ec54973829c5cf.png",
    },
    'Подвеска': {
        'tasks': {
        ...
        'image': "d:/py/suspension.png",
    },
    'Тормоза': {
        'tasks': {
            'Замена тормозной жидкости': 100,
        },
        'image': "https://moto-detal.ru/content/uploads/2019/07/133-1-1.jpg",
    },
}
...
# Функция для создания кругового прогрессбара
def create_progress_bar(source, title):
    p = figure(width=200, height=200, title=title, tools="")
    p.annular_wedge(
        x=0, y=0, inner_radius=0.3, outer_radius=0.5,
...
# Прогрессбары
progress_bars = {
    task: create_progress_bar(progress_sources[task], task)
...
service_inputs = {
    task: TextInput(title=f"Когда было последнее '{task}'?", value=str(maintenance_data.get(task, "")))
    for category in categories.values() for task in category['tasks'].keys()
}
...
for category, data in categories.items():
    ...
    bars = row(*[progress_bars[task] for task in tasks.keys() if task in progress_bars])
    
    # Изображение
    img_div = Div(text=f"<img src='{image}' alt='{category}' style='width:150px; height:auto; margin-bottom: 20px;'>")
    
    # Аннотация
    annotation = Div(text=f"<div style='font-size: 18pt; color: #007acc;'><b>{category}</b></div>", width=900)
...
layout = column(
    Div(text="<h1 style='color: #007acc; text-align: center;'>Дашборд для обслуживания эндуро мотоцикла</h1>"),
    hours_input,

...
for category, tasks in categories.items():
    inputs = column(*[service_inputs[task] for task in tasks.keys()])
    bars = row(*[progress_bars[task] for task in tasks.keys()])
    annotation = Div(text=f"<div style='font-size: 14pt; color: blue;'>Рекомендации для {category.lower()}:</div>")
    section = column(annotation, inputs, bars)
